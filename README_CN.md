...
Triggerix 是一个完整的**事件-条件-动作 (ECA) 规则引擎生态**，能够在运行时构建并驱动任意交互逻辑。由多个协作的 npm 包和应用组成，核心设计理念是：将规则表示为数据，支持可视化编辑、验证和运行时执行。

本 Demo 项目 (triggerix-lang.github.io) 是 Triggerix 的官方演示网站，展示完整的规则编辑与执行流程。
...
| triggerix-lang.github.io     | Demo 应用 | 官方演示网站，展示完整的规则编辑和执行流程   |
...
编辑器基于 War3 地图编辑器的触发器面板设计，每条规则包含三个区域：
...
每个工具有一个 `resolve` 函数，将用户输入转换为规则引擎可理解的值。
...
graph TD
    ...
    Core --> Schema["@triggerix/schema<br/>规则构建 API"]
    Core --> Runtime["@triggerix/runtime<br/>运行时引擎"]
    Core --> Editor["@triggerix/editor<br/>通用编辑器框架"]
    Core --> Validator["@triggerix/validator<br/>规则验证"]
...
| 包名                     | 版本   | 职责                       | 关键导出                                                                                                     |
| ------------------------ | ------ | -------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `@triggerix/core`        | v0.0.3 | 类型定义、接口规范         | Event / Condition / Action / Rule / Expression / ActionNode                                                  |
| `@triggerix/schema`      | v0.0.3 | 规则构建 API               | defineEvent / defineAction / defineCondition / defineRule / expr / sequence / parallel / tryCatch / actionIf |
| `@triggerix/runtime`     | v0.0.3 | 规则引擎执行               | createRuntime / 事件分发 / evaluateCondition / executeActionNode / ExpressionEvaluator                       |
| `@triggerix/editor`      | v0.0.3 | 通用编辑器抽象（框架无关） | Editor 接口 / 描述符系统 / 可观察状态                                                                        |
| `@triggerix/validator`   | v0.0.3 | 规则验证                   | 规则结构与类型校验                                                                                           |
...
├── definitions/              # 规则定义（描述 Demo 的触发器规则）
│   ├── button-click.ts
...
```mermaid
flowchart LR
    Defs["definitions<br/>规则定义"] --> Editor3["useTriggerEditor<br/>可视化编辑器"]
    Editor3 -->|onChange 触发| Sync["syncRules()<br/>生成规则 JSON"]
...
1. **定义阶段**：`definitions/` 中定义每个 Demo 的事件、动作、工具、规则模板
2. **编辑阶段**：`trigger-ui/` 提供可视化编辑器，用户可交互编辑触发器规则
3. **同步阶段**：编辑器变更通过 `onChange` → `syncRules()` 生成规则 JSON
4. **执行阶段**：`useDemoRuntime` 将规则加载到 `@triggerix/runtime`，分发事件时自动匹配和执行
...
| 构建层   | `@triggerix/schema`            | 规则构建 API        |
| 验证层   | `@triggerix/validator`         | 规则验证            |
| 运行层   | `@triggerix/runtime`           | 规则引擎执行        |
