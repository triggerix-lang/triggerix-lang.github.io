...
> 用户说：「我要修改偏好」
>
> AI 生成一个偏好设置表单组件，包含：饮食偏好（荤/素/清真/无忌口）、口味辣度（不辣/微辣/中辣）、饮食禁忌（无/葱蒜/海鲜）等多个选项。开发者只需定义业务逻辑处理（如验证、保存偏好），Triggerix 和 AI 则负责编排组件的结构、样式和交互事件，并在运行时驱动整体流程。

当前阶段，Triggerix 已实现用 JSON Schema 描述任意交互事件，并在运行时驱动执行。

![Triggerix 示例](.github/images/example.png)
